...
console.log('\n== мок GL: путь отрисовки ==');
{
  ...
  {
    // Кадр к кадру число вызовов гуляет, пока подтягиваются плитки
    // поверхности, поэтому сперва ждём, когда оно устаканится: иначе
    // проверка будет мигать, а мигающая проверка хуже отсутствующей.
    const settle = () => {
      let prev = -1, same = 0;
      for (let i = 0; i < 80 && same < 4; i++) {
        const d = frame();
        same = d === prev ? same + 1 : 0;
        prev = d;
      }
      return prev;
    };
    ...
    game.peers = [Object.assign({}, near, {
      pos: v3(ship.pos.x + 5000, ship.pos.y, ship.pos.z),
    })];
    const withFar = frame();
