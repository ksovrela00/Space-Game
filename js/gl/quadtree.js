 ...
 *
 * Плитка дробится, пока её геометрическая ошибка (то, что сетка этого
 * уровня не может показать) в пикселях больше допуска. Дробление
 * происходит ТОЛЬКО если все четыре потомка уже готовы: иначе рисуется
 * сама плитка. Поэтому в кадре никогда не бывает дырок, а подгрузка
 * выглядит как постепенное уточнение.
 *
 * @param ctx {radius, camPos, camDir(локальное направление на камеру),
 *             camAlt, focal, tol, maxLevel, errorOf(level), ready(t), want(t, err)}
...
export function selectTiles(ctx, out = []) {
  ...
  while (stack.length && guard++ < 20000) {
    ...
    const err = ctx.errorOf(t.level) * R * ctx.focal / dist;
