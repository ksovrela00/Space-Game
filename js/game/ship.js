...
export const SHIP = {
  ...
  // Восстановление ВДВОЕ короче расхода: форсаж задуман на отрыв от
  // грунта и на маневр у станции, то есть на повторение через минуту, а
  // не на один раз за вылет. При пятнадцати секундах между двумя
  // рывками приходилось ждать дольше, чем длится сам рывок.
  ...
  maxHull: 100,
  // Щит: невидимая оболочка, принимающая урон вперёд корпуса. Видно её
  // только в момент попадания — постоянное свечение вокруг корабля
  // превратило бы бой в дискотеку, а разглядеть в нём что-то стало бы
  // невозможно.
  maxShield: 40,
  // Восстанавливается сам, но не сразу: пауза нужна, чтобы в перестрелке
  // щит не отрастал между двумя выстрелами и бой не превращался в
  // бесконечный. Числа отсюда идут и на сервер (tools/export.mjs): он
  // считает щит по тем же правилам, и разойтись им негде.
  shieldRegen: 4,     // единиц в секунду
  shieldDelay: 5,     // с тишины, прежде чем начнёт восстанавливаться
  // Трюм меряется ТОННАМИ, а не кубометрами: объём в накладной никого
  // не интересует, а масса — то, что потом будет влиять на разгон.
  hold: 20,           // т
  // Бак. Расхода пока нет ни у одного привода, и бак всегда полон: это
  // честнее, чем рисовать убывающую шкалу, за которой ничего не стоит.
  // Появится расход — здесь же будет и он.
  fuelMax: 12,        // т
