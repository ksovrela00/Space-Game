...
/**
 * Квантовый привод: ПУСТ ДО ЗАГРУЗКИ.
 *
 * Числа приходят из бэкенда вместе с остальным снаряжением — привод там
 * заведён модулем `quantum` (server/data/specs.php). Скорость прыжка при
 * этом принадлежит КОРПУСУ и приезжает в лётной модели: появятся другие
 * корабли — у каждого будет своя.
 */
export const QUANTUM = {
  // Два числа остались здесь, и оба не настройки.
  //
  // `segs` — на сколько кусков режется трасса при проверке коридора.
  // Внутри куска тело считается неподвижным, а расстояние до отрезка
  // берётся аналитически. Точками трассу проверять НЕЛЬЗЯ: на перелёте в
  // полтора миллиона километров даже полсотни точек стоят в 30 000 км
  // друг от друга, и планета целиком проваливается между ними. Это
  // параметр алгоритма, а не свойство привода.
  segs: 12,
  // `relief` — оболочка рельефа: самый рваный тип поверхности
  // поднимается на 1.7% радиуса (js/gl/terrain.js), и запас взят с
  // походом. Свойство генератора поверхности, и меняться оно обязано
  // вместе с ним, а не из базы.
  relief: 0.02,
};
...
/**
 * Принять числа привода.
 *
 * Допуск по прицелу приходит в ГРАДУСАХ и переводится здесь же — по той
 * же причине, что и конус доворота у пушки: градусы подкручивает
 * человек, радианами считает тригонометрия.
 */
export function applyQuantumSpec(spec) {
  Object.assign(QUANTUM, spec);
  QUANTUM.align = (spec.alignDeg || 0) * DEG;
  // Скорость по умолчанию — для тех, у кого своей нет (ship.quantumSpeed
  // главнее). Приезжает она из лётной модели корпуса, см. `reads`.
  QUANTUM.speed = spec.quantumSpeed;
  return QUANTUM;
}
