...
/**
 * Перенести корабль вместе с телом за шаг мира dt.
 *
 ...
 *
 * DT ЗДЕСЬ — ШАГ МИРА, А НЕ КАДРА, и перепутать их стоило дорого. Часы
 * мира подводятся к серверным ходом, до четверти скорости
 * (js/game/clock.js), а после спящей вкладки — рывком на десятки секунд.
 * Корабль, перенесённый на кадровый шаг, каждый кадр отстаёт от грунта
 * на разницу. Гонит её ВРАЩЕНИЕ, а не орбита: тела здесь ползут по
 * орбитам сантиметры в секунду, а грунт на экваторе идёт под две с
 * половиной сотни метров. Четверть хода — это шестьдесят с лишним
 * метров в секунду мимо грунта (то самое дёрганье земли), а рывок в
 * минуту оставляет корабль в пятнадцати километрах от места.
 *
 * `moved` — измеренное смещение тела (замер до и после updateWorld).
 * Оно нужно ради рывка: скорость на время даёт там хорду вместо дуги, а
 * замер верен при любом шаге и не стоит ничего.
 *
 * @param dt шаг МИРА, а не кадра
 * @param moved измеренное смещение тела за этот шаг или null
 * @returns вес, с которым скомпенсировано вращение (нужен тем, кто
 *          считает скорость относительно грунта)
 */
export function carryShip(ship, body, dt, moved = null) {
  ship.pos.x += moved ? moved.x : body.vel.x * dt;
  ship.pos.y += moved ? moved.y : body.vel.y * dt;
  ship.pos.z += moved ? moved.z : body.vel.z * dt;
