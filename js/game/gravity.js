 ...
 * установившаяся скорость просадки.
 *
 * Просадка появляется только с выпущенным шасси. С убранным высоту
 * держит компенсатор — иначе стыковка превратилась бы в борьбу с
 * притяжением планеты, а гравитация нужна там, где она имеет смысл: на
 * посадке.
 */
export function gravityField(body, ship) {
  ...
  return {
    up: _fieldUp,
    sink: ship.gear.t > 0.02 ? gravityAt(body, ship.pos) * SHIP.sinkTime : 0,
  };
}
