...
// Доведение с ОГРАНИЧЕННЫМ шагом: за кадр не больше step.
//
// В отличие от approach это поведение тела с ограниченной силой: скорость
// меняется линейно, а не экспонентой. Разница вся в начале — экспонента
// в первый же миг даёт максимальное ускорение (то есть рывок), а
// постоянная сила разгоняет ровно. Именно поэтому вращение корабля
// считается так, а не через approach.
export const rampTo = (cur, target, step) =>
