...
export function showHelp(game) {
  show(`
    ...
    <table class="rows">
      ...
      <tr><td>J</td><td class="v">варп-привод: прыжок в другую систему</td></tr>
      ...
      <tr><td>I</td><td class="v">меню пилота: корабль, груз, задания, финансы</td></tr>
      <tr><td>в меню: 1–4, &larr;/&rarr;, ЛКМ</td><td class="v">раздел; I или Esc — закрыть</td></tr>
      ...
      <tr><td>на карте: G</td><td class="v">карта галактики / карта системы</td></tr>
      <tr><td>на карте: Tab</td><td class="v">назначить выбранное целью (в галактике система становится целью варпа сразу при выборе)</td></tr>
    ...
    <p class="sub">Перелёт в другую систему — варп-привод. Порядок такой:
      открыть карту (<b>M</b>), переключиться на галактику (<b>G</b>) и выбрать
      систему — щелчком или <b>&larr;</b>/<b>&rarr;</b>. Выбранная система сразу
      становится целью варпа. Дальше в
      полёте <b>J</b>: в кадре появится кольцо, показывающее направление на
      ту звезду, и его надо совместить с носом. Через четыре секунды
      удержания корабль рвёт с места и уходит в тоннель на двадцать-тридцать
      секунд; прервать прыжок нельзя. Выход — прямо у чужой звезды, в
      четырёх её радиусах.</p>
    <p class="sub">Старой системы во время прыжка не существует: она
      выгружается из памяти целиком, а новая собирается под тоннелем. Поэтому
    ...
    <p class="sub"><b>Другие пилоты</b> видны тремя способами сразу:
      кораблём в кадре, оранжевым квадратом с именем и расстоянием и
      отметкой на сканере (рядом с кольцом — их число). Видно только тех,
      кто в той же звёздной системе.</p>

    <p class="sub"><b>Бой.</b> <b>Tab</b> берёт в цель то, на что наведён
      нос, — в том числе чужой корабль. <b>Левая кнопка мыши</b> — огонь.
      Лазерная пушка стоит <b>на кардане</b>: она сама доворачивает ствол
      к упреждённой точке — туда, где цель окажется, когда болт долетит.
      Отметка прицела зелёная, пока кардан дотягивается, и жёлтая, когда
      он упёрся в предел: тогда доворачивать надо самому. Дальность —
      2.5 км, дальше болт гаснет.</p>

    <p class="sub">К выбранному пилоту работает и <b>квантовый привод</b>
      (<b>B</b>): выход — за <b>20 км</b> от него, а не в упор, и с его же
      ходом, если он летит обычно. Ушёл в прыжок сам — выйдем на нуле: за
      таким вектором лететь некуда. Пропал с локатора посреди калибровки —
      прыжок срывается, а не идёт к призраку.</p>

    <p class="sub">Первым удар принимает <b>щит</b>. Его не видно вовсе —
      кроме мига попадания: оболочка вспыхивает по форме корпуса, ярче
      всего там, куда пришёл луч. Щит отрастает сам через несколько
      секунд тишины, а вот корпус чинят за деньги в порту. Свои корпус и
      щит — в левом нижнем углу, чужие — полосками над квадратом пилота.</p>

    <p class="sub">Урон считает сервер: попадание видит стрелявший, а
      корпус и щит меняются только на сервере, и обе стороны узнают о них
      оттуда. Сбитый корабль восстанавливается в том порту, откуда пилот
      последний раз уходил, — платы за это пока нет.</p>

    <p class="sub">В левом верхнем углу — <b>связь</b>: полоски показывают
      качество, число рядом — задержку до сервера. Появились
      <b>потери</b> — часть снимков не доходит, и чужие корабли будут
      дёргаться. Там же видно и обрыв: «СВЯЗЬ ОБОРВАНА» — своя игра при
      этом продолжается, пропадают только чужие.</p>
    ...
    <p class="sub">Меню пилота (<b>I</b>) открывается только в полёте и
      ...
      глохнет — клавиши разбирает меню. Четыре раздела: <b>корабль</b>
      (габариты, состояние корпуса, бак, установленные модули),
      <b>груз</b> (сколько тонн занято и чем), <b>задания</b> (срок и
      награда) и <b>финансы</b> (лента: сколько пришло и сколько ушло в
      кронах). Груз и задания пока выданы стартовым набором — брать их
      будет негде, пока на станциях не появятся рынок и доска.</p>
    ...
    <p class="sub">На телефоне управление сенсорное: слева джойстик — он ведёт
      нос (вниз по экрану — нос вверх, как штурвал на себя), справа ползунок
      тяги, между ними кнопки — форсаж, крен, подъёмные движки, шасси, цель,
      прыжок. Палец по свободному месту вертит камеру. Полноэкранного режима
      на iPhone нет: чтобы убрать адресную строку, добавьте игру на экран
      «Домой». На ПК полный экран — кнопка с уголками в правом верхнем углу.</p>
